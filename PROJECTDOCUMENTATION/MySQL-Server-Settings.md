...
While every project, and organization has their own guidelines on database servers, we require the following settings to ensure Dotz Enterprise Platform runs smoothely.
...
Timezones on the following elements should be UTC for Dotz Enterprise Platform to function correctly:
...
4) The User interacting with the Dotz Enterprise Platform. The user's activities and timestamps will be automatically converted on the front-end by our application. What the user sees, interms of timestamps, will always be in local time.
